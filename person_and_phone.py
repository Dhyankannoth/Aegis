...
def load_darknet_weights(model, weights_file):
    ...
    for layer_name in layers:
        ...
        for i, layer in enumerate(sub_model.layers):
            if not layer.name.startswith('conv2d'): continue
            batch_norm = None
...
def yolo_nms(outputs, anchors, masks, classes):
    ...
    boxes, scores, classes, nums = tf.image.combined_non_max_suppression(
        ...
        max_output_size_per_class=100, max_total_size=100, iou_threshold=0.5, score_threshold=0.6
    )
...
def DarknetBlock(x, filters, blocks):
    ...
    for _ in range(blocks): x = DarknetResidual(x, filters)
    return x
...
def YoloConv(filters, name=None):
    def yolo_conv(x_in):
        ...
        x = DarknetConv(x, filters, 1); x = DarknetConv(x, filters * 2, 3)
        x = DarknetConv(x, filters, 1); x = DarknetConv(x, filters * 2, 3)
        x = DarknetConv(x, filters, 1)
...
def get_yolo_model(weights_path='models/yolov3.weights'):
    yolo = YoloV3()
    load_darknet_weights(yolo, weights_path)
    return yolo

def detect_objects(image, yolo_model, class_names):
...
if __name__ == "__main__":
    ...
    while True:
        ret, image = cap.read()
        if not ret: break
        boxes, scores, classes, nums = detect_objects(image, yolo, class_names)
